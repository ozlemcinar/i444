...
class Sensors {
  ...
  static async newSensors(mongoDbUrl) {
    //@TODO
	///(mongodb:\/\/)+((([0-9]+\.[0-9]\.[0-9]\.[0-9])|[a-zA-Z]+)|([a-zA-Z]+\.[a-zA-Z]+\.[a-zA-Z]+))\:[0-9]+(\/)?([a-zA-Z]+)?
	var matching = mongoDbUrl.match(/(mongodb:\/\/)+((([0-9]+\.[0-9]\.[0-9]\.[0-9])|[a-zA-Z]+)|([a-zA-Z]+\.[a-zA-Z]+\.[a-zA-Z]+))\:[0-9]+(\/)?([a-zA-Z]+)?/);	
	if(matching==null){
		const err = `no model  sensor type`;
          	throw [ new AppError('X_ID', err) ];	
	}
    	else{
          	//console.log('a');
		var subMatchings = mongoDbUrl.split('/');
		var subsubMatchings = subMatchings[0].split(':');
		var hostName = (subsubMatchings[0]);
		var portName = subMatchings[3];	
		const client = new mongo(mongoDbUrl,{useNewUrlParser:true, useUnifiedTopology:true});

		client.connect(function(err) {
  			assert.equal(null, err);
  			console.log("Connected successfully to server");
  			const db = client.db(portName);
			new Sensors(client);
		});
		return new Sensors();
	}
  ...
  async addSensorType(info) {
	...
	this.sensorTypes[sensorType._id] = sensorType;
    //@TODO
  }
  ...
  async addSensor(info) {
    ...
    //@TODO
	if (sensor._model && !this.sensorTypes[sensor._model]) {
      		throw [ `unknown model "${sensor.model}"` ];
    	}
    	this.sensors[sensor._id] = sensor;
  }
  ...
  async addSensorData(info) {
    ...
    //@TODO
	if (sensorData._sensorId && !this.sensors[sensorData._sensorId]) {
      		throw `unknown sensor id "${sensorData.sensorId}"`;
    	}
    	let data = this.sensorData[sensorData._sensorId];
    	const timestamp = Number(sensorData._timestamp);
    	if (!data) {
      		data = {
			sensorId: sensorData._sensorId,
			earliest: timestamp,
			latest: timestamp,
			data: {},
      		};
      		this.sensorData[sensorData._sensorId] = data;
    	}
...
function findInCollection(collection, searchSpecs, name) {
  ...
  if (searchSpecs._id) {
	console.log('2');
    const value = collection[searchSpecs._id];
    if (value) {
	console.log('3');
      data.push(value);
    }
